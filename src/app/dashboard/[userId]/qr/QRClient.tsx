    ...
    const setupQR = async () => {
      try {
        const { data: profile, error: fetchError } = await supabase
          .from('profiles')
          .select('wallet_address, base_amount_unit')
          .eq('id', userId)
          .single();

        if (fetchError) throw new Error('Failed to load profile');
        if (!profile?.wallet_address || !profile?.base_amount_unit) {
          throw new Error('Missing wallet address or amount');
        }

        const { wallet_address, base_amount_unit } = profile;

        const encodedData = encodeFunctionData({
          abi: erc20Abi,
          functionName: 'transfer',
          args: [wallet_address, base_amount_unit],
        });
